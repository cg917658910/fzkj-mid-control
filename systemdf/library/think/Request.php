...
class Request
{
    ...
    /**
     * 获取客户端IP地址
     * @param boolean $trustProxy  是否进行高级模式获取（有可能被伪装）
     * @return mixed
     */
    public function ip($trustProxy = true)
    {
        // 从ThinkPHP的Request对象获取服务器变量
        $request = \think\Request::instance();

        // 如果不信任代理，直接返回REMOTE_ADDR
        if (!$trustProxy) {
        ...
        foreach ($headers as $header) {
            $ipList = $request->header(strtolower($header));
            if (!empty($ipList)) {
                // 处理X-Forwarded-For格式：client, proxy1, proxy2
                if ($header === 'X-Forwarded-For') {
                    $ips = explode(',', $ipList);
                    foreach ($ips as $ip) {
                        $ip = trim($ip);
                        if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
                            return $ip;  // 返回第一个非私有IP
                        }
                    }
                } else {
                    // 其他头通常只包含一个IP
                    if (filter_var($ipList, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
                        return $ipList;
                    }
                }
            }
        }
        // 如果没有可信的代理头，返回服务器直接连接的IP
        return $request->server('REMOTE_ADDR') ?: '0.0.0.0';
    }
